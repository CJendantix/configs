...
enum ParseRangeError {
    ...
    SplitError { unsplittable_string: String },

    #[snafu(display("Too many colons in {unsplittable_string}"))]
    TooManyValuesError { unsplittable_string: String },

    #[snafu(display("Failed to parse {unparseable_integer_string} into an integer"))]
    ParseError { source: ParseIntError, unparseable_integer_string: String }
}

fn parse_range(argument: &str) -> Result<RangeInclusive<usize>, ParseRangeError> {
    let found: Vec<&str> = argument.split(':').collect();
    if found.len() > 2 {
        return Err(ParseRangeError::TooManyValuesError { unsplittable_string: argument.to_owned() })
    }
    if found.len() == 0 {
        return Err(ParseRangeError::SplitError { unsplittable_string: argument.to_owned() })
    }
