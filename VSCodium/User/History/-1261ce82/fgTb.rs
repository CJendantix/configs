...
fn validate_input(input: &str, wrong_guesses_remaining: usize, wrong_guesses: &[char], correct_guesses: &[char]) -> Result<(), &'static str>{
    let input = input.to_lowercase();
    let mut error = "";

    if !input.chars().any( |c| wrong_guesses.contains(&c) || correct_guesses.contains(&c) )
    {
        error = "You gave a previously revealed answer, ya dunce!";
    }
    Err(error)
}
...
fn main() {
    ...
    loop {
        ...
        if num_occurances == 0 {
            wrong_guesses.extend(guess.chars());
            println!("Wrong!");
            sleep(Duration::from_secs(1));
        } else {
            for _ in 0..num_occurances {
                correct_guesses.extend(guess.chars());
            }
            println!("Correct!");
            sleep(Duration::from_secs(1));
        }
