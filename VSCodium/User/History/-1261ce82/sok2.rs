...
enum ParseRangeError {
    ...
    SplitError { unsplittable_string: &'static str },

    #[snafu(display("Too many colons in {unsplittable_string}"))]
    TooManyValuesError { unsplittable_string: &'static str },

    #[snafu(display("Failed to parse {unparseable_integer_string} into an integer"))]
    ParseError { source: ParseIntError, unparseable_integer_string: &'static str }
}

fn parse_range(argument: &str) -> Result<RangeInclusive<usize>, ParseRangeError> {
    let found: Vec<&str> = argument.split(':').collect();
    if found.len() > 2 {
        return Err(ParseRangeError::TooManyValuesError { unsplittable_string: argument })
    }
    if found.len() == 0 {
        return Err(ParseRangeError::SplitError { unsplittable_string: argument })
    }
    
    let (start, end) = 
    let [start, end] = [start, end].map(|s| s.parse::<usize>().with_context(|_| ParseSnafu { unparseable_integer_string: s}));
    Ok(start?..=end?)
}
